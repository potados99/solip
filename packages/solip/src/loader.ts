...
export default class Loader<T> {
  ...
  /**
   * modelDir 아래에 있는 모듈들(=TS 소스코드)에 변경이 생기면 다시 불러오도록 합니다.
   * 변경 감지 -> 트랜스파일 -> require 캐시 삭제 -> 임포트 순서입니다.
   */
  autoReload() {
    console.log(chalk.blue(`Solip API 루트 경로: ${Solip.apiRootPath}`));
    console.log(chalk.blue(`모듈 경로: ${this.modelDir}`));
